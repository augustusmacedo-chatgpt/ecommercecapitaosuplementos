...
function isRequiredField(element: Element) {
  return element instanceof HTMLInputElement || element instanceof HTMLSelectElement || element instanceof HTMLTextAreaElement;
}
function validValue(element: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
  return element.value.trim() !== '';
}
function clearFieldError(field: HTMLElement) {
  field.classList.remove('checkout-field-invalid');
  const wrapper = field.closest('label');
  wrapper?.querySelector('.checkout-field-error')?.remove();
}
function showFieldError(field: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
  ...
  message.className = 'checkout-field-error';
...
export default function CheckoutValidation() {
  useEffect(() => {
    ...
    style.textContent = `
      .checkout-page label em { display:none !important; }
      .checkout-page .checkout-field-invalid { border-color:#c62828 !important; box-shadow:0 0 0 2px rgba(198,40,40,.08) !important; }
      .checkout-page .checkout-field-error { display:block; margin-top:6px; color:#c62828; font-size:12px; font-weight:700; line-height:1.35; }
    `;
    ...
    const validate = (event: Event) => {
      ...
      if (!(target instanceof HTMLFormElement) && !(target instanceof HTMLButtonElement)) return;
    ...
    const clearOnInput = (event: Event) => {
      ...
      if (validValue(field)) clearFieldError(field);
    };
    ...
    return () => {
      ...
      document.removeEventListener('click', validate, true);
      document.removeEventListener('input', clearOnInput, true);
      document.removeEventListener('change', clearOnInput, true);
      style.remove();
    };
  }, []);
  return null;
