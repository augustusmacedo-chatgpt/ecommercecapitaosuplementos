...
export default function AdminCatalogControl() {
  ...
  const runSync = useCallback(async (restart = false) => {
    ...
    try {
      ...
      const data = await readJson(`/api/bling/catalog-sync${query}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }) as ActionResult;
  ...
  return (
    <main style={{ minHeight: '100vh', padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 14, marginBottom: 16 }}>
        <div style={card}><Database size={18} /><div style={{ marginTop: 10, opacity: .7, fontSize: 12 }}>Índice R2</div><strong style={{ fontSize: 24 }}>{health?.catalog?.indexedProducts ?? '—'}</strong><div style={{ opacity: .65, marginTop: 4 }}>{health?.catalog?.source || 'aguardando leitura'}</div></div>
        ...
        <div style={card}><ShieldCheck size={18} /><div style={{ marginTop: 10, opacity: .7, fontSize: 12 }}>Concorrência</div><strong>{health?.concurrency?.active ? 'BLOQUEADO' : health ? 'LIVRE' : '—'}</strong><div style={{ opacity: .65, marginTop: 4 }}>{health?.rateLimit?.cooldownActive ? 'Cooldown do Bling ativo' : 'Sem cooldown ativo'}</div></div>
      </section>
      ...
      <section style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button type="button" onClick={() => runSync(true)} disabled={busy} style={button}><RefreshCw size={16} /> Iniciar novo snapshot</button>
        </div>
