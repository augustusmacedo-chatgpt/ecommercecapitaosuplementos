...
import { ArrowLeft, Banknote, Check, CreditCard, FileText, LogOut, Menu, Minus, Plus, QrCode, Search, ShoppingCart, Sparkles, Trash2, UserRound, X } from 'lucide-react';

type Product = { id: number; name: string; code?: string; ean?: string; price: number; image?: string; imageCandidates?: string[]; stock: number; stockByLocation: { camapua: number; newfit: number } };
...
function imageCandidatesOf(product: any) {
  const raw = [
    product?.imagemOriginal,
    product?.originalImage,
    ...(Array.isArray(product?.images) ? product.images.filter((image: any) => image?.type === 'original').map((image: any) => image?.url) : []),
    product?.imagemURL,
    ...(Array.isArray(product?.imagens) ? product.imagens.map((image: any) => typeof image === 'string' ? image : image?.link || image?.url) : []),
    ...(Array.isArray(product?.midia?.imagens?.externas) ? product.midia.imagens.externas.map((image: any) => image?.link || image?.url) : []),
    ...(Array.isArray(product?.midia?.imagens?.internas) ? product.midia.imagens.internas.flatMap((image: any) => [image?.link || image?.url, image?.linkMiniatura]) : []),
    product?.imagemMiniatura,
    product?.thumbnailImage,
  ];
  const seen = new Set<string>();
  return raw.map((value: any) => String(value || '').trim()).filter((value: string) => {
...
function mapCatalogProducts(data: any): Product[] {
  ...
  return source.map((p: any) => {
    ...
    return {
      ...
      // Mantém qualquer padrão GTIN retornado pelo Bling: EAN-13, UPC/GTIN-12 etc.
      ean: p.ean || p.gtin || p.gtinProduto || p.codigoBarras || p.codigoEAN || p.gtinTributario,
      price: parsePrice(p.preco), image: imageCandidates[0], imageCandidates,
...
export default function Pdv({ initialLocation = 'camapua', lockLocation = false, onLogout }: { initialLocation?: LocationKey; lockLocation?: boolean; onLogout?: () => void | Promise<void> }) {
  ...
  const [stockFilter, setStockFilter] = useState<'inStock' | 'outOfStock'>('inStock');
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter(product => {
      const localStock = Number(product.stockByLocation[location] || 0);
      const matchesStock = stockFilter === 'inStock' ? localStock > 0 : localStock <= 0;
  ...
  }, [products, query, sortMode, location, stockFilter]);
  ...
  return <div className="pdv-shell">
    ...
    <header className="pdv-top">
      ...
      {menuOpen && <nav className="pdv-menu"><button onClick={() => { setCustomerPanel(true); setMenuOpen(false); }}><UserRound size={15}/> CONSULTAR CLIENTE</button><button onClick={() => go('orders')}><ShoppingCart size={15}/> CONSULTAR VENDAS</button><button onClick={() => go('nfc')}><FileText size={15}/> CONSULTAR NFC-e</button><button onClick={() => { setOtherStoreOpen(true); setMenuOpen(false); }}><Search size={15}/> CONSULTAR ESTOQUE</button><button onClick={() => { window.dispatchEvent(new CustomEvent('pdv-cash-handoff')); setMenuOpen(false); }}><span style={{display:'inline-grid',placeItems:'center',width:15,height:15,border:'1px solid currentColor',borderRadius:'50%',fontSize:9}}>↻</span> TROCA DE CAIXA</button><button onClick={() => { window.dispatchEvent(new CustomEvent('pdv-cash-close')); setMenuOpen(false); }}><Banknote size={15}/> FECHAMENTO TOTAL DO CAIXA</button>{onLogout && <><div className="pdv-menu-divider"/><button className="pdv-menu-logout" onClick={async () => { setMenuOpen(false); await onLogout(); }}><LogOut size={15}/> SAIR DO PDV</button></>}</nav>}
    </header>
