...
function ProductCard({ product, onAdd }: { product: SiteProduct; onAdd: (product: SiteProduct, quantity: number) => void }) {
  ...
  return <article className={`product-card ${unavailable ? 'product-unavailable' : ''}`}>
    ...
    <div className="product-body">
      ...
      <button className={`product-button ${unavailable ? 'disabled' : ''}`} onClick={() => !unavailable && onAdd(product, 1)} disabled={unavailable}>{unavailable ? 'INDISPONÍVEL' : 'ADICIONAR À SACOLA'}</button>{!unavailable && <a className="product-detail-link" href={`/produto/${product.id}`}>VER PRODUTO</a>}
    </div>
  </article>;
}
