...
function CheckoutFlowBridge() {
  useEffect(() => {
    ...
    updateCheckoutButton(); updateOrderActions(); const observer = new MutationObserver(location.pathname === '/' ? updateCheckoutButton : updateOrderActions); observer.observe(document.body, { childList: true, subtree: true }); return () => { observer.disconnect(); window.clearInterval(timer); };
  }, []); return null;
}
