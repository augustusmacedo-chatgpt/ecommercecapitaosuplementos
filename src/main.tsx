...
function Page(){
  ...
  if(location.pathname==='/checkout'){
    if(!hasValidCheckoutSession())return <CheckoutIdentityPage/>;
    const total=(()=>{try{return JSON.parse(localStorage.getItem('capitao-cart')||'[]').reduce((sum:number,entry:{price?:string;quantity?:number;product?:{price?:string;quantity?:number}})=>{const item=entry.product||entry;const n=String(item.price||'').replace(/[^0-9,]/g,'').replace(/\./g,'').replace(',','.');const v=Number(n);const q=Math.max(1,Number(entry.quantity||item.quantity||1));return sum+(Number.isFinite(v)?v*q:0)},0)}catch{return 0}})();
    return <><CheckoutPage/><CheckoutLoyalty checkoutId={sessionStorage.getItem('capitao-checkout-id')||''} total={total}/></>;
  }
