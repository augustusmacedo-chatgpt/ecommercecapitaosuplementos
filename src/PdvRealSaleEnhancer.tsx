...
export default function PdvRealSaleEnhancer() {
  useEffect(() => {
    ...
    const readCartFromDom = () => {
      const nodes = Array.from(document.querySelectorAll<HTMLElement>('.pdv-cart-item'));
    ...
    const readCustomer = () => {
      const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('.pdv-shell input'));
      ...
      const document = values.find(x => x.placeholder.includes('cpf') || x.placeholder.includes('cnpj') || /^\d[\d.\-/ ]{10,}$/.test(x.value))?.value || '';
      const name = values.find(x => x.placeholder.includes('nome'))?.value || values.find(x => x.type === 'text' && x.value && x.value !== document && x.value !== phone && x.value !== email)?.value || '';
      return { name, document, phone, email };
    };

    const readLocation = () => {
      const select = document.querySelector<HTMLSelectElement>('.pdv-location select');
    ...
    const readDocumentChoice = () => {
      const overlay = document.querySelector<HTMLElement>('.pdv-overlay');
      if (!overlay) return documentChoice;
      const selected = overlay.querySelector<HTMLElement>('.pdv-doc.selected');
      if (!selected) return documentChoice;
      const text = normalize(selected.innerText || '');
      return text.includes('nfc') ? 'nfc' : 'receipt';
    };

    const flash = (message: string, error = false) => {
      const old = document.getElementById('pdv-real-sale-feedback');
      old?.remove();
      const box = document.createElement('div');
      box.id = 'pdv-real-sale-feedback';
    ...
    const submit = async (button: HTMLButtonElement) => {
      ...
      submitting = true;
      setButtonState(button, 'REGISTRANDO VENDA...', true);
      ...
      try {
        ...
        flash(error instanceof Error ? error.message : 'Não foi possível registrar a venda.', true);
      }
    };
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (card) {
        ...
        if (product) { const current = tracked.get(product.id); tracked.set(product.id, { product, quantity: (current?.quantity || 0) + 1 }); }
        return;
      }
      const step = target.closest<HTMLElement>('.pdv-step button');
      if (step) {
        const itemNode = step.closest<HTMLElement>('.pdv-cart-item');
        const name = normalize(itemNode?.querySelector('strong')?.textContent || '');
        const product = products.find(p => normalize(p.name) === name) || products.find(p => name.includes(normalize(p.name)));
        if (product) { const current = tracked.get(product.id); const delta = normalize(step.textContent || '').includes('+') ? 1 : -1; tracked.set(product.id, { product, quantity: Math.max(0, (current?.quantity || 1) + delta) }); }
        return;
      }
      const doc = target.closest<HTMLElement>('.pdv-doc');
      if (doc) { const text = normalize(doc.innerText || ''); documentChoice = text.includes('nfc') ? 'nfc' : 'receipt'; return; }
      const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>('.pdv-overlay button'));
      const button = target.closest<HTMLButtonElement>('button');
      if (!button || !buttons.includes(button)) return;
      const text = normalize(button.innerText || '');
      if (text.includes('cancelar') || text.includes('voltar') || text === 'x') return;
