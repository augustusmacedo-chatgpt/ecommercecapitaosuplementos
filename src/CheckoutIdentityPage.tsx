...
export default function CheckoutIdentityPage() {
  ...
  async function sendCode() {
    ...
    setStep('code');
    setMessage(`Código enviado para ${data.maskedEmail || 'seu e-mail'}.`);
  }
  ...
  async function verify() {
    ...
    if (mode === 'email') {
      ...
      localStorage.setItem('capitao-verified-email', identifier.trim().toLowerCase());
    } else {
      localStorage.removeItem('capitao-verified-email');
      localStorage.setItem('capitao-verified-document', identifier.replace(/\D/g, ''));
    }
  ...
  return <main className="identity-page container"><div className="identity-card">
    ...
    <div className="identity-icon"><ShieldCheck size={25}/></div>
    <span className="identity-eyebrow">COMPRA SEGURA</span>
    <h1>Finalize sua venda</h1>
    <p>{step === 'identify' ? 'Confirme sua identidade para continuar sua compra.' : 'Digite o código recebido no seu e-mail.'}</p>
    {step === 'identify' ? <>
      <div className="identity-mode"><button type="button" className={mode==='document'?'active':''} onClick={()=>switchMode('document')}>CPF / CNPJ</button><button type="button" className={mode==='email'?'active':''} onClick={()=>switchMode('email')}><Mail size={14}/> E-MAIL</button></div>
      <div className="identity-hint">{mode==='document' ? 'Já possui cadastro? Consulte pelo CPF ou CNPJ.' : 'Ainda não possui cadastro? Use seu e-mail para receber o código.'}</div>
      <input type={mode==='email'?'email':'text'} placeholder={mode==='email'?'Digite seu e-mail':'Digite seu CPF/CNPJ'} value={identifier} onChange={e=>setIdentifier(mode==='email'?e.target.value:maskDocument(e.target.value))} autoComplete="email" />
      <button className="auth-submit" onClick={sendCode} disabled={loading || !valid}>{loading?'ENVIANDO CÓDIGO...':'ENVIAR CÓDIGO'}</button>
    </> : <>
      <div className="identity-destination">Código enviado para <strong>seu e-mail</strong>.</div>
