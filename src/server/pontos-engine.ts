...
async function loadAccount(customerKey: string) {
  try {
    ...
    return emptyPointsAccount(customerKey);
  }
}
...
function orderDocument(order: OrderSnapshot) {
  return order.customerDocument || order.contato?.numeroDocumento || order.customer?.document || order.cliente?.numeroDocumento || '';
}

function orderEmail(order: OrderSnapshot) {
  return String(order.customerEmail || order.email || order.customer?.email || order.contato?.email || '').trim().toLowerCase();
}

export function isEligibleOrderStatus(status: unknown) {
  const text = typeof status === 'object' && status !== null ? String((status as any).descricao || (status as any).nome || (status as any).valor || '') : String(status || '');
  const normalized = text.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  return ['entregue', 'finalizado', 'concluido', 'concluida'].some(word => normalized.includes(word));
}

export function isCancelledOrderStatus(status: unknown) {
  const text = typeof status === 'object' && status !== null ? String((status as any).descricao || (status as any).nome || (status as any).valor || '') : String(status || '');
  const normalized = text.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  return normalized.includes('cancel') || normalized.includes('devol') || normalized.includes('estornado');
}
...
export async function awardOrderPoints(order: OrderSnapshot) {
  ...
  const points = pointsFromOrderTotal(total);
  if (!points) return { earned: false, reason: 'zero-points' as const };
  ...
  if (next.cycleEarned >= CYCLE_POINTS) {
    const cycle = Math.floor((next.lifetimeEarned - 1) / CYCLE_POINTS) + 1;
    next = await queueBonusEmail(next, cycle);
    const bonus = createBonusRecord(cycle, next);
    next = { ...next, balance: Math.max(0, next.balance - CYCLE_POINTS), cycleEarned: next.cycleEarned - CYCLE_POINTS, entries: [...next.entries, { id: `cycle-${cycle}-conversion`, type: 'redeem', points: -CYCLE_POINTS, description: `Conversão automática do ciclo ${cycle} em bônus de R$ 50,00`, createdAt: new Date().toISOString() }], lifetimeRedeemed: next.lifetimeRedeemed + CYCLE_POINTS, bonuses: [...(next.bonuses || []).filter(item => item.id !== bonus.id), bonus], updatedAt: new Date().toISOString() };
  }
...
export async function reverseOrderPoints(order: OrderSnapshot) {
  ...
  const next = { ...applyEntry(account, { type: 'reversal', points: -Math.abs(earned.points), orderId, checkoutId, description: `Estorno dos pontos da compra #${orderId}` }), entries: [...applyEntry(account, { type: 'reversal', points: -Math.abs(earned.points), orderId, checkoutId, description: `Estorno dos pontos da compra #${orderId}` }).entries.slice(0, -1), { id: reversalId, type: 'reversal' as const, points: -Math.abs(earned.points), orderId, checkoutId, description: `Estorno dos pontos da compra #${orderId}`, createdAt: new Date().toISOString() }] };
  await saveAccount(next);
  await flushPendingPointEmails(next);
