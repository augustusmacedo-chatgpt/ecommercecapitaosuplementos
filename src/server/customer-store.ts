...
export async function loadCustomer(email: string) { if (!hasStorage()) throw new Error('Armazenamento de clientes não configurado no Cloudflare R2.'); const result = await get(customerKey(email)); if (!result?.stream) return null; try { return JSON.parse(await new Response(result.stream).text()) as CustomerRecord; } catch { return null; } }
export async function saveCustomer(customer: CustomerRecord) { if (!hasStorage()) throw new Error('Armazenamento de clientes não configurado no Cloudflare R2.'); await put(customerKey(customer.email), JSON.stringify(customer), { contentType: 'application/json' }); }
