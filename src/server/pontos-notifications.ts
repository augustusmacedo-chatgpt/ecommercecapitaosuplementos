...
async function save(account: PointsAccount) {
  ...
  await put(key(account.customerKey), JSON.stringify(account), { access: 'private', addRandomSuffix: false, allowOverwrite: true, contentType: 'application/json', ...(token ? { token } : {}) });
}
...
export async function queueMilestoneEmails(account: PointsAccount, previousCycleEarned: number) {
  ...
  for (const milestone of [200, 400, 600, 800]) {
