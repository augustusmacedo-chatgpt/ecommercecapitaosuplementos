import { BLING_REDIRECT_URI, STATE_COOKIE, clearCookie, json, parseCookies } from './bling-shared.js';
...
export default async function handler(request: Request) {
  ...
  try {
    ...
    await saveStoredData({ ...config, refreshToken: tokens.refresh_token });
    ...
    return fail(err instanceof Error ? err.message : 'Não foi possível concluir a conexão com o Bling.', 502);
  }
}
