import { CONFIG_COOKIE, sendJson, parseCookies, readJsonBody, seal, unseal, type BlingConfig } from './shared.js';
...
export default async function handler(request: any, response: any) {
  try {
    ...
    if (!clientId || !clientSecret) return sendJson(response, { error: 'Client ID e Client Secret são obrigatórios.' }, 400);

    const value: BlingConfig = { clientId, clientSecret, inviteLink };
    const sealed = await seal(value);
    return sendJson(response, { ok: true, configured: true, clientId: value.clientId, secretConfigured: true, inviteLink: value.inviteLink }, 200, {
    ...
    return sendJson(response, { error: 'Não foi possível carregar a configuração do Bling.' }, 500);
  }
}
