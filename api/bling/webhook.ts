...
async function rawRequestBody(request: RequestLike) {
  ...
  if (body && typeof body === 'object' && typeof (body as { getReader?: unknown }).getReader === 'function') return await new Response(body as ReadableStream).text();
...
async function loadOrder(checkoutId: string) {
  try {
    ...
    return JSON.parse(await new Response(result.stream).text()) as Record<string, any>;
  } catch { return null; }
}
async function saveOrder(checkoutId: string, value: Record<string, any>) {
  const token = process.env.BLOB_READ_WRITE_TOKEN;
    ...
    if (checkoutId && isOrderEvent) {
      ...
      if (current) {
        const next = {
          ...
          numero: Number(data.numero || current.numero || 0) || current.numero,
          data: data.data || current.data || null,
          total: Number(data.total || current.total || 0),
          situacao: data.situacao || current.situacao || null,
          contato: data.contato || current.contato || null,
          loja: data.loja || current.loja || null,
          vendedor: data.vendedor || current.vendedor || null,
          updatedAt: new Date().toISOString(),
          webhookEvent: payload.event || null,
        };
        await saveOrder(checkoutId, next);
        console.info('Pedido sincronizado pelo webhook:', checkoutId, next.situacao, next.vendedor);
      }
    }
