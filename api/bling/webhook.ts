      ...
      if (current) {
        ...
        if (isCancelledOrderStatus(status)) {
          if (next.pointsAwarded && !next.pointsReversed) {
            ctx?.waitUntil(reverseOrderPoints({ ...next, checkoutId }).then(async result => { if (result.reversed || result.duplicate) await saveOrder(checkoutId, { ...next, pointsReversed: true, pointsReversedAt: next.pointsReversedAt || new Date().toISOString(), pointsReversalResult: result }); console.info('Estorno de pontos da compra processado:', checkoutId, result); }).catch(error => console.error('Estorno de pontos da compra:', checkoutId, error)));
          }
          if (Number(next.loyaltyPointsRedeemed || 0) > 0 && !next.loyaltyPointsReversed) {
            ctx?.waitUntil(reverseOrderRedemption({ ...next, checkoutId }).then(async result => { if (result.reversed || result.duplicate) await saveOrder(checkoutId, { ...next, loyaltyPointsReversed: true, loyaltyPointsReversedAt: next.loyaltyPointsReversedAt || new Date().toISOString(), loyaltyPointsReversalResult: result }); console.info('Estorno de resgate de pontos processado:', checkoutId, result); }).catch(error => console.error('Estorno do resgate de pontos:', checkoutId, error)));
          }
        } else if (isEligibleOrderStatus(status) && !next.pointsAwarded && !next.pointsReversed) {
          ctx?.waitUntil(awardOrderPoints({ ...next, checkoutId }).then(async result => { if (result.earned || result.duplicate) await saveOrder(checkoutId, { ...next, pointsAwarded: true, pointsAwardedAt: next.pointsAwardedAt || new Date().toISOString(), pointsAwardedResult: result }); console.info('Pontos processados para pedido:', checkoutId, result); }).catch(error => console.error('Processamento de pontos do pedido:', checkoutId, error)));
        }
      }
    }
