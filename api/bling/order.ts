...
function pointsBlobOptions() { const token = process.env.BLOB_READ_WRITE_TOKEN; return { access: 'private' as const, useCache: false, ...(token ? { token } : {}) }; }
async function loadPointsAccount(customerKey: string) { try { const result = await get(`points/accounts/${createHash('sha256').update(customerKey).digest('hex')}.json`, pointsBlobOptions()); if (!result?.stream) return emptyPointsAccount(customerKey); const parsed = JSON.parse(await new Response(result.stream).text()); return { ...emptyPointsAccount(customerKey), ...parsed, customerKey, entries: Array.isArray(parsed.entries) ? parsed.entries : [], reservations: Array.isArray(parsed.reservations) ? parsed.reservations : [], bonuses: Array.isArray(parsed.bonuses) ? parsed.bonuses : [] } as PointsAccount; } catch { return emptyPointsAccount(customerKey); } }
async function savePointsAccount(account: PointsAccount) { const token = process.env.BLOB_READ_WRITE_TOKEN; await put(`points/accounts/${createHash('sha256').update(account.customerKey).digest('hex')}.json`, JSON.stringify(account), { access: 'private', addRandomSuffix: false, allowOverwrite: true, contentType: 'application/json', ...(token ? { token } : {}) }); }

export async function POST(request: Request) {
  if (request.method !== 'POST') return json({ error: 'Método não permitido.' }, 405);
  try {
    const body = await readJsonBody(request) as Input;
    const checkoutId = clean(body.checkoutId); const customer = body.customer || {}; const document = digits(customer.document); const email = normalizeEmail(customer.email); const items = Array.isArray(body.items) ? body.items : []; const reservationId = clean(body.loyalty?.reservationId);
    ...
    if (reservationId) {
      const sessionCustomerKey = verifySession(request);
      if (!sessionCustomerKey || sessionCustomerKey !== customerKey) return json({ error: 'A sessão de fidelidade não corresponde ao cliente do pedido.' }, 401);
      const account = cleanupExpiredReservations(await loadPointsAccount(customerKey));
      const reservation = (account.reservations || []).find(item => item.id === reservationId && item.checkoutId === checkoutId && item.status === 'reserved');
      if (!reservation) return json({ error: 'A reserva de pontos não está disponível ou expirou.' }, 409);
      loyaltyReservation = { id: reservation.id, points: reservation.points, value: reservation.value };
    }
    ...
    const subtotal = normalizedItems.reduce((sum, item) => sum + item.quantidade * item.valor, 0);
    const loyaltyDiscount = loyaltyReservation ? Math.min(loyaltyReservation.value, subtotal) : 0;
    ...
    const accountBeforeSave = loyaltyReservation ? cleanupExpiredReservations(await loadPointsAccount(customerKey)) : null;
    let pointsAccount = accountBeforeSave;
    if (loyaltyReservation && pointsAccount) {
      const reservation = (pointsAccount.reservations || []).find(item => item.id === loyaltyReservation!.id && item.checkoutId === checkoutId && item.status === 'reserved');
      if (!reservation) return json({ error: 'O pedido foi criado, mas a reserva de pontos não pôde ser localizada para baixa automática.' }, 500);
      pointsAccount = applyEntry(pointsAccount, { type: 'redeem', points: -reservation.points, orderId, checkoutId, description: `Resgate de ${reservation.points} pontos no pedido ${orderNumber || orderId}` });
      pointsAccount = { ...pointsAccount, reservations: (pointsAccount.reservations || []).map(item => item.id === reservation.id ? { ...item, status: 'consumed' as const } : item), updatedAt: new Date().toISOString() };
      await savePointsAccount(pointsAccount);
    }
    await saveCreatedOrder(checkoutId, { id: orderId, numero: orderNumber, checkoutId, customerKey, customerDocument: document, customerEmail: email, customerName: clean(customer.name), data: today(), total, subtotal, loyaltyDiscount, loyaltyReservationId: loyaltyReservation?.id || null, loyaltyPointsRedeemed: loyaltyReservation?.points || 0, pointsEligible: true, pointsAwarded: false, pointsReversed: false, situacao: { valor: 'Em aberto' }, vendedor: null, itens: normalizedItems.map(item => ({ produtoId: item.produto.id, descricao: item.descricao || 'Produto', quantidade: item.quantidade, valor: item.valor, total: item.quantidade * item.valor })), estimatedPoints: pointsFromOrderTotal(total), updatedAt: new Date().toISOString() });
    return json({ created: true, orderId, orderNumber, blingContactId: contactId, subtotal, loyaltyDiscount, total, loyaltyPointsRedeemed: loyaltyReservation?.points || 0 }, 201);
