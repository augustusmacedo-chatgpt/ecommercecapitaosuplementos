...
export async function POST(request: Request) {
  ...
  try {
    ...
    const orderResponse = await fetch(`${BLING_BASE}/pedidos/vendas`, { method: 'POST', headers, body: JSON.stringify(orderPayload) }); const orderText = await orderResponse.text(); if (!orderResponse.ok) { console.error('Bling sales order rejected:', orderResponse.status, orderText.slice(0, 1500)); return json({ error: `O Bling rejeitou o pedido. ${blingError(orderText)}`.trim() }, orderResponse.status === 401 || orderResponse.status === 403 ? 403 : 422); }
    let result: { data?: { id?: number; numero?: number } } = {}; try { result = JSON.parse(orderText); } catch { /* resposta inesperada */ } const orderId = Number(result.data?.id || 0); const orderNumber = Number(result.data?.numero || 0) || undefined; if (!orderId) return json({ error: 'O Bling recebeu uma resposta sem o ID do pedido.' }, 502);
    ...
    await saveCreatedOrder(checkoutId, { id: orderId, numero: orderNumber, checkoutId, customerKey, customerId: customerKey, customerEmail: email, customerName: clean(customer.name), data: today(), total, subtotal, loyaltyDiscount, loyaltyReservationId: loyaltyReservation?.id || null, loyaltyPointsRedeemed: loyaltyReservation?.points || 0, pointsEligible: true, pointsAwarded: false, pointsReversed: false, situacao: { valor: 'Em aberto' }, vendedor: null, itens: normalizedItems.map(item => ({ produtoId: item.produto.id, descricao: item.descricao || 'Produto', quantidade: item.quantidade, valor: item.valor, total: item.quantidade * item.valor })), estimatedPoints: pointsFromOrderTotal(total), updatedAt: new Date().toISOString() });
