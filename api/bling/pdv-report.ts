...
const STORE_DEFINITIONS = [
  { key: 'CAMAPUA', name: 'CAMAPUÃ', stock: 'ESTOQUE MATRIZ' },
  { key: 'NEWFIT', name: 'NEWFIT', stock: 'ESTOQUE NEWFIT' },
];
...
function isAttended(order: AnyRecord) {
  ...
  const labels = [situation.nome, situation.descricao, situation.valor, order?.situacaoNome, order?.status];
  return labels.some(value => normalize(value) === 'ATENDIDO');
}
...
function storeFor(order: AnyRecord) {
  const candidates = [
    ...
    order?.loja?.nome,
    order?.unidadeNegocio?.descricao,
...
async function blingGet(token: string, path: string) {
  ...
  if (!response.ok) {
    ...
    (error as AnyRecord).status = response.status;
    throw error;
  }
  return body;
}
